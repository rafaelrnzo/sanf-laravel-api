...
class SendEmailProductAddToCartForAdminJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $mailable = (new MailLayout2Columns())
            ...
            ->line(__('Pengguna atas SANFIND atas nama <strong>“' . $this->data['fullName'] . '”</strong> telah mengajukan pengajuan pembiayaan kendaraan dengan detail sebagai berikut'))
            ->writeContent($this->data['content'])
            ->generateSeparator([
                ['joinToIndex' => 2, 'html' => '<hr style="border: 1px solid rgba(3, 37, 126, 0.08); margin: 5px 0;">'],
            ])
            ->lineWithUrl(
                __('Pengajuan ini adalah bagian dari kerjasama SANFIND & Scanina.com <br><br> Jika Anda merasa tidak membuat request tersebut mohon abaikan email ini atau anda dapat'),
