...
use Sanf\Core\Modules\Scanina\Services\GuzzleBrowseProductFilterBrandService;

class BrowseProductSparePartBrandController extends RestApiController
{
    public function __invoke(Request $request, Guard $userAuth, GuzzleBrowseProductFilterBrandService $service)
    {
        ...
        $productFilterBrandRequestDto = new BrowseProductFilterRequestDto(
            $queryParam + [
                'user_id' => $userAuth->id(),
        ...
        $result = $service->execute($productFilterBrandRequestDto);
