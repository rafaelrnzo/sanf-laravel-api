...
class ProcessSurveySubmissionService implements ApplicationServiceInterface
{
    private SurveyRepositoryInterface $repository;
    private EnrichSurveySubmissionLocationsService $locationService;

    public function __construct(
        SurveyRepositoryInterface $repository,
        EnrichSurveySubmissionLocationsService $locationService
    ) {
    ...
    public function execute($dto = null)
    {
        ...
        $inputs['xid'] = nano_id();
        $items = $this->locationService->execute($dto->items);

        $inputs['items'] = collect($items)->map(function ($data) use ($dto) {
            $imageFiles = null;
            $imagePaths = null;
