...
final class HandleMidtransCallbackUseCase
{
    ...
    public function execute(MidtransWebhookPayload $payload): PaymentModel
    {
        ...
        $this->updateMidtransTransaction($midtransTransaction, $payload);

        $paymentUpdatePayload = [];

        if ($payment->status === PaymentStatusEnum::EXPIRE_IN_PROGRESS) {
            $paymentUpdatePayload = $this->handlePaymentExpireInProgress($payment, $payload);
        }

        if ($payment->status === PaymentStatusEnum::PENDING) {
            $paymentUpdatePayload = $this->handlePaymentPending($payment, $payload);
        }
    ...
    private function updateMidtransTransaction(
        ...
        $updatePayload = array_filter([
            ...
            'transaction_time' => Carbon::parse($payload->transaction_time, MidtransClient::TIMEZONE),
            'fraud_status' => $payload->fraud_status,
            'gross_amount' => $payload->gross_amount,
        ], static fn ($value) => $value !== null);

        $updatePayload['raw_response'] = $payload->raw_request;
    ...
    private function handlePaymentExpireInProgress(PaymentModel $payment, MidtransWebhookPayload $payload)
    {
        ...
        if (
            in_array($payload->transaction_status, [
    ...
    private function handlePaymentPending(PaymentModel $payment, MidtransWebhookPayload $payload)
    {
        ...
        if (
            ...
            || $payload->transaction_status === MidtransTransactionStatusEnum::EXPIRE
        ) {
            $newPaymentStatus = PaymentStatusEnum::EXPIRED;
