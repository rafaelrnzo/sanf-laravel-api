...
class SendPushNotificationInstallmentAlmostDueJob implements ShouldQueue
{
    ...
    public function handle(
        ...
        $dueDateShortFormat = $dueDateCarbon->locale('id')->translatedFormat('j M');

        $user = $userRepository->find([
            'xid' => $userProfileXid,
        ]);

        $title = __('Tagihan Anda Hampir Jatuh Tempo');
        $subtitle = __('Informasi');
        $body = "Tagihan Anda akan jatuh tempo pada {$dueDateShortFormat} dengan nilai {$totalAmountCurrency}. Bayar sekarang sebelum terkena biaya keterlambatan.";
        $bodyHtml = "<span>Tagihan Anda akan jatuh tempo pada <b>{$dueDateShortFormat}</b> dengan nilai <b>{$totalAmountCurrency}</b>. Bayar sekarang sebelum terkena biaya keterlambatan.</span>";
