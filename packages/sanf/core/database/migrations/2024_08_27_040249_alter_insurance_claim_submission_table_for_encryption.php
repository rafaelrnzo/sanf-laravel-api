...
class AlterInsuranceClaimSubmissionTableForEncryption extends Migration
{
    protected $tableName = 'insurance_claim_submission';
    protected $util;
    ...
    {
        $this->util->changeTypeToText($this->tableName, 'location_metadata');
    }

    private function changeFieldsToBytea(): void
    {
        $this->util->changeTypeToBytea($this->tableName, 'location_metadata');
        $this->util->changeComment($this->tableName, 'location_metadata', 'json');
    }

    private function changeBackFieldsType(): void
    {
        $this->util->changeTypeToJson($this->tableName, 'location_metadata');
        $this->util->removeComment($this->tableName, 'location_metadata');
    }
        ...
        DB::table($this->tableName)->chunkById(500, function ($data) {
            foreach ($data as $item) {
                ...
                DB::table($this->tableName)
                    ->where('id', $item->id)
                    ->update([
                        'location_metadata' => $encryption->decryptFromHex($item->location_metadata),
                    ]);
            }
        });
    }
