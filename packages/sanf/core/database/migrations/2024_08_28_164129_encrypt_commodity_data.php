...
class EncryptCommodityData extends Migration
{
    ...
    public function up()
    {
        DB::connection(ConnectionDB::PG_SQL)->table('commodity')->chunkById(500, function ($data) {
            foreach ($data as $item) {
                $encryption = SodiumEncryption::encryptor();

                DB::connection(ConnectionDB::PG_SODIUM)->table('commodity_encrypted')
                    ->insert(
                        array_merge(
                            (array) $item,
                            [
        ...
        MigrationPostgreSqlUtil::make(ConnectionDB::PG_SODIUM)->rearrangeSequence('commodity_encrypted');
    }
