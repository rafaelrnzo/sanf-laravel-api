...
class EncryptEsignOtpData extends Migration
{
    ...
    public function up()
    {
        DB::connection(ConnectionDB::PG_SQL)->table('esign_otp')->chunkById(500, function ($data) {
            foreach ($data as $item) {
                $encryption = SodiumEncryption::encryptor();

                DB::connection(ConnectionDB::PG_SODIUM)->table('esign_otp_encrypted')
                    ->insert(
                        array_merge(
                            (array) $item,
                            [
        ...
        MigrationPostgreSqlUtil::make(ConnectionDB::PG_SODIUM)->rearrangeSequence('esign_otp_encrypted');
    }
