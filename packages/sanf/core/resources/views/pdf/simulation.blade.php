        ...
        <tr>
            <td class="container">
                ...
                <div class="content">
                    ...
                    <table class="another-email-content" border="0" cellpadding="0" cellspacing="0" width="100%">
                        @foreach($contents['data'] as $key => $value)
                            @php
                                $key = ucwords(str_replace('_', ' ', $key));
                                $key === 'Persen Dp' ? $key = 'Persen DP' : $key;
                                $key === 'Dp ( Uang Muka )' ? $key = 'DP ( Uang Muka )' : $key;
                            @endphp
                            ...
                            @else
                                <tr>
                                    @if($key == 'Total Biaya - Biaya')
                                    <td align="left" width="50%" class="font-weight-bold" style="font-size: 16px;">{{ $key }}</td>
                                    @else
                    ...
                    @if($contents['summary'])
                    <table class="another-email-content" border="0" cellpadding="0" cellspacing="0" width="100%" style="padding-bottom: 25px; border-top: 1px solid #d9d9d9">
                        @foreach($contents['summary'] as $key => $value)
                            @php
                                $key = ucwords(str_replace('_', ' ', $key));
                            @endphp
                            ...
                            @else
                                @if($key == 'Total Angsuran Pertama')
                                <tr>
                                    <td align="left" width="50%" class="font-weight-normal">{{ $key }}</td>
                                    <td align="right" width="50%" class="font-weight-bold">{{ $value }}</td>
                                </tr>
                                <tr><td class="notes" colspan="2">Total pembayaran pertama belum termasuk biaya notaris atau fiducia</td></tr>
                                @else
                                <tr>
                                    <td align="left" width="50%" class="font-weight-normal">{{ $key }}</td>
                                    <td align="right" width="50%" class="font-weight-bold">{{ $value }}</td>
